...
export default function ShoeCabinet() {
  ...
  function handleFile(e) {
    ...
    reader.onload = ev => { setPreviewImg(ev.target.result); setForm(f => ({ ...f, image: ev.target.result })); };
  ...
  function handleSaveEdit() {
    if (!editForm.name || !editForm.brand) return;
            ...
            <div style={S.formGrid}>
              {[["name", "球鞋名稱 *"], ["brand", "品牌 *"], ["colorway", "配色"], ["size", "尺寸 (US)"], ["year", "年份"]].map(([key, label]) => (
                <div key={key} style={S.formField}>
                  ...
                  <input style={S.formInput} value={editForm[key] || ""} onChange={e => setEditForm(f => ({ ...f, [key]: e.target.value }))} placeholder={label.replace(" *", "")} />
                </div>
              ))}
          ...
          <div style={S.formModal} onClick={e => e.stopPropagation()}>
            ...
            <div style={S.formGrid}>
              {[["name", "球鞋名稱 *"], ["brand", "品牌 *"], ["colorway", "配色"], ["size", "尺寸 (US)"], ["year", "年份"]].map(([key, label]) => (
                <div key={key} style={S.formField}>
                  <label style={S.formLabel}>{label}</label>
                  <input style={S.formInput} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder={label.replace(" *", "")} />
                </div>
              ))}
              <div style={S.formField}>
                <label style={S.formLabel}>標籤</label>
            ...
            {(!form.name || !form.brand) && (
              <div style={{ fontSize: 11, color: "#bbb", marginTop: 14, textAlign: "center" }}>請填寫球鞋名稱及品牌</div>
            )}
            <button
              style={{ ...S.addBtn, width: "100%", marginTop: 12, justifyContent: "center", opacity: (!form.name || !form.brand) ? 0.4 : 1, cursor: (!form.name || !form.brand) ? "not-allowed" : "pointer" }}
              onClick={handleAdd}
            >
              加入收藏
            </button>
...
const S = {
  ...
  cardImgWrap: { position: "relative", aspectRatio: "4/3", overflow: "hidden", background: "#f7f7f5" },
  ...
  shelfImg: { width: 68, height: 48, objectFit: "cover", borderRadius: 8, background: "#f4f4f2", flexShrink: 0 },
  ...
  modalImgWrap: { background: "#f7f7f5", aspectRatio: "16/10", overflow: "hidden" },
