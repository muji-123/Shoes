...
export default function ShoeCabinet() {
  ...
  async function handleAdd() {
    try {
      const data = await sbFetch("shoes", {
        method: "POST",
        prefer: "return=representation",
        body: JSON.stringify({ name: form.name, brand: form.brand, year: form.year, size: form.size, colorway: form.colorway, tag: form.tag, image: form.image }),
      });
      setShoes(prev => [...prev, data[0]]);
      setForm({ name: "", brand: "", year: "", size: "", colorway: "", tag: "經典", image: "" });
      setPreviewImg(null); setShowForm(false);
      showToast("已加入收藏");
    } catch { showToast("新增失敗，請重試", "warn"); }
  }
