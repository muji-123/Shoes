...
const SUPABASE_URL = "https://vhkuczgkbuzyhztwlnaw.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZoa3VjemdrYnV6eWh6dHdsbmF3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkxOTk3NjksImV4cCI6MjA5NDc3NTc2OX0.6tEg1x6nI6g73oBjy-vxCM7LDStPncGJK7Cik3gt6Cg";

const TAGS = ["全部", "原版", "復刻", "球員版", "聯名", "未市售"];
const SHOE_TAGS = ["原版", "復刻", "球員版", "聯名", "未市售"];
...
export default function ShoeCabinet() {
  ...
  useEffect(() => {
    sbFetch("shoes?select=*&order=created_at.asc")
      .then(data => { setShoes(data || []); setLoading(false); })
      .catch(() => { setLoading(false); showToast("載入失敗，請重新整理", "warn"); });
  }, []);
  ...
  const years = ["全部", ...Array.from(new Set(shoes.map(s => s.year).filter(Boolean))).sort((a,b) => b-a)];
  const filtered = shoes.filter(s => {
    const matchFilter = filterMode === "tag"
      ? (filter === "全部" || s.tag === filter)
      : (filter === "全部" || s.year === filter);
  ...
  async function handleAdd() {
    try {
      // Compress image further if too large (keep under 500KB)
      ...
      const res = await fetch(`${SUPABASE_URL}/rest/v1/shoes`, {
        method: "POST",
        headers: {
          "apikey": SUPABASE_KEY,
          "Authorization": `Bearer ${SUPABASE_KEY}`,
        ...
        body: JSON.stringify({ name: form.name, brand: form.brand, year: form.year, size: form.size, colorway: form.colorway, tag: form.tag, image: imageData }),
      });
      if (!res.ok) {
      ...
      setShoes(prev => [...prev, data[0]]);
      setForm({ name: "", brand: "", year: "", size: "", colorway: "", tag: "原版", image: "" });
      setPreviewImg(null); setShowForm(false);
      showToast("已加入收藏");
    } catch(e) { showToast("新增失敗：" + e.message, "warn"); }
  }

  async function handleDelete(id) {
    try {
      await sbFetch(`shoes?id=eq.${id}`, { method: "DELETE" });
      setShoes(prev => prev.filter(s => s.id !== id));
      setSelected(null); showToast("已移除收藏", "warn");
    } catch { showToast("刪除失敗，請重試", "warn"); }
  }
  ...
  async function handleSaveEdit() {
    try {
      await sbFetch(`shoes?id=eq.${editForm.id}`, {
        method: "PATCH",
        prefer: "return=representation",
        body: JSON.stringify({ name: editForm.name, brand: editForm.brand, year: editForm.year, size: editForm.size, colorway: editForm.colorway, tag: editForm.tag, image: editForm.image }),
      });
      setShoes(prev => prev.map(s => s.id === editForm.id ? editForm : s));
      setSelected(editForm);
      setEditMode(false); setEditForm(null); setEditPreview(null);
      showToast("已儲存變更");
    } catch { showToast("儲存失敗，請重試", "warn"); }
  }
      ...
      <div style={S.controlRow}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <button style={{ ...S.tagBtn, ...(filterMode === "tag" ? S.tagActive : {}) }} onClick={() => { setFilterMode("tag"); setFilter("全部"); }}>標籤</button>
            <button style={{ ...S.tagBtn, ...(filterMode === "year" ? S.tagActive : {}) }} onClick={() => { setFilterMode("year"); setFilter("全部"); }}>年份</button>
            {filterMode === "year" && (
              <select style={{ ...S.formInput, marginLeft: 4, padding: "5px 10px", fontSize: 13 }} value={filter} onChange={e => setFilter(e.target.value)}>
            ...
            <div style={S.formGrid}>
              ...
              <div style={S.formField}>
                ...
                <select style={S.formInput} value={editForm.tag} onChange={e => setEditForm(f => ({ ...f, tag: e.target.value }))}>
                  {SHOE_TAGS.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
            </div>
...
const S = {
  ...
  cardImgWrap: { position: "relative", background: "#f7f7f5", display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  uploadArea: { border: "1.5px dashed #d0d0cc", borderRadius: 10, minHeight: 90, flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", marginBottom: 16, overflow: "hidden", background: "#f5f5f3", transition: "border-color 0.2s" },
  uploadPlaceholder: { display: "flex", alignItems: "center", gap: 10, color: "#bbb" },
  uploadPreview: { width: "100%", height: "auto", display: "block" },
